    ...
    def device_config(self) -> tuple:
        if torch.cuda.is_available():
            ...
            if (
                ...
                print("16 series/10 series GPUs and P40 forced to single precision")
                self.is_half = False
            ...
            print("No supported NVIDIA GPU found, using MPS for inference")
            self.device = "mps"
        else:
            print("No supported NVIDIA GPU found, using CPU for inference")
            self.device = "cpu"
            self.is_half = True
...
def get_vc(model_path):
    ...
    del net_g.enc_q
    print(net_g.load_state_dict(cpt["weight"], strict=False))  # Odd that this line is needed to clean up
